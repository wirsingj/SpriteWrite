...
- Ollama provider changes: `src/providers/ollamaPatchProvider.test.ts` includes a live local `qwen3:14b` integration check when Ollama is running and that model is installed. It can take about a minute. If aborted Ollama requests leave qwen busy, unload it with a local `/api/generate` call using `{"model":"qwen3:14b","keep_alive":0}` before rerunning.
