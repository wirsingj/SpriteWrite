...
AI Assist follows the same validated-edit model. Provider output becomes proposed edit JSON, then validation, then preview, then accept or reject. Whole-animation drafts are stricter: they must validate and pass basic coherence checks before SpriteWrite stages them for Apply Draft / Reject Draft, so invalid model output never half-mutates the project and valid drafts do not replace rows until the user applies them.
...
- Valid broad animation drafts show a compact staged draft review with Apply Draft and Reject Draft controls before project mutation.
